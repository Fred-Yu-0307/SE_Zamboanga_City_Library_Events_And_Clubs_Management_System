...
document.addEventListener('DOMContentLoaded', function () {
	...
	statusFilter.addEventListener('change', function () {
		...
		rows.forEach(function (row) {
			...
			const statusCell = row.querySelector('td:nth-child(9)'); // Adjust the index according to your table structure
...
//modal description
document.addEventListener('DOMContentLoaded', function() {
    const textareas = document.querySelectorAll('.auto-expand-scrollable');
    
    function adjustTextareaHeight(textarea) {
        // Reset height to get correct scrollHeight
        textarea.style.height = 'auto';
        
        // Calculate new height (not exceeding max-height from CSS)
        const minHeight = parseInt(getComputedStyle(textarea).minHeight);
        const scrollHeight = textarea.scrollHeight;
        
        // Apply height (will be constrained by max-height from CSS)

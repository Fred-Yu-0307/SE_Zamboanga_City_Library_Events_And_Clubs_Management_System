...
class Events{
    ...
    public function edit() {
        ...
        // Calculate eventStatus if not provided
        if ($this->eventStatus === null) {
            $currentDateTime = date('Y-m-d H:i:s');
            $eventStartDateTime = date('Y-m-d H:i:s', strtotime($this->eventStartDate . ' ' . $this->eventStartTime));
            $eventEndDateTime = date('Y-m-d H:i:s', strtotime($this->eventEndDate . ' ' . $this->eventEndTime));
        ...
        try {
            ...
            $stmt->execute([
                ...
                ':eventGuestLimit' => $this->eventGuestLimit,
                ':eventRegion' => $this->eventRegion,
                ':eventProvince' => $this->eventProvince,
                ':eventCity' => $this->eventCity,
                ':eventBarangay' => $this->eventBarangay,
                ':eventStreetName' => $this->eventStreetName,
                ':eventBuildingName' => $this->eventBuildingName,
            ...
            foreach ($this->organizationClubIDs as $orgClubID) {
                $stmt = $conn->prepare("INSERT INTO event_orgclub (eventID, organizationClubID) VALUES (:eventID, :organizationClubID)");
                $stmt->execute([
                    ':eventID' => $this->eventID,
                    ':organizationClubID' => $orgClubID
                ]);
            }
    ...
    public function fetch($eventID) {
        ...
        // Get event details - explicitly list all fields to ensure they're included
        $stmt = $conn->prepare("SELECT 
            eventID, eventTitle, eventDescription, 
            eventStartDate, eventEndDate, 
            eventStartTime, eventEndTime, 
            eventGuestLimit, eventRegion, 
            eventProvince, eventCity, 
            eventBarangay, eventStreetName, 
            eventBuildingName, eventZipCode, 
            eventStatus 
        ...
        // Get facilitator IDs as array
        $stmt = $conn->prepare("SELECT librarianID FROM event_facilitator WHERE eventID = :eventID");
        $stmt->execute([':eventID' => $eventID]);
        $event['librarianIDs'] = $stmt->fetchAll(PDO::FETCH_COLUMN);
        
        // Get organization IDs as array
        $stmt = $conn->prepare("SELECT organizationClubID FROM event_orgclub WHERE eventID = :eventID");
        $stmt->execute([':eventID' => $eventID]);
        $event['organizationClubIDs'] = $stmt->fetchAll(PDO::FETCH_COLUMN);
    ...
    public function getAllEvents() {
        $sql = "SELECT eventID, eventTitle, eventStartDate, eventEndDate, eventStartTime, eventEndTime FROM event";
        $query = $this->db->connect()->prepare($sql);
    ...
    public function updateEventStatus($eventID, $newStatus) {
        $sql = "UPDATE event SET eventStatus = :status, eventUpdatedAt = NOW() WHERE eventID = :eventID";
    ...
    public function getAllEventsExcept($eventID) {
        $sql = "SELECT eventID, eventStartDate, eventEndDate, eventStartTime, eventEndTime 
                FROM event 
                WHERE eventID != :eventID";
        $query = $this->db->connect()->prepare($sql);
    ...
    public function hasConflictingEvent($newStart, $newEnd, $otherEvents) {
        $newStartTime = strtotime($newStart);
        $newEndTime = strtotime($newEnd);
    
        foreach ($otherEvents as $event) {
            // Combine date and time of the other event
            $existingStart = strtotime($event['eventStartDate'] . ' ' . $event['eventStartTime']);
            $existingEnd = strtotime($event['eventEndDate'] . ' ' . $event['eventEndTime']);
    
            // Check if the events are at the same location
            $sameLocation = (
                $event['eventRegion'] == $this->eventRegion &&
                $event['eventProvince'] == $this->eventProvince &&
                $event['eventCity'] == $this->eventCity &&
                $event['eventBarangay'] == $this->eventBarangay &&
                $event['eventStreetName'] == $this->eventStreetName &&
                $event['eventBuildingName'] == $this->eventBuildingName &&
                $event['eventZipCode'] == $this->eventZipCode
            );
    
            // If same location, check for time overlap
            if ($sameLocation &&
